...
async function main() {
  ...
  const newAdmin=await request(`/api/platform/v1/tenants/${tenantId}/quick-admin`,{name:'测试管理员',phone:'13800000001'},platform);
  ...
  const first=await request('/api/auth/login',{email:newAdmin.data.credentials.phone,password:newAdmin.data.credentials.password});
  ...
  ok((await request('/api/auth/login',{email:newAdmin.data.credentials.phone,password:newAdmin.data.credentials.password})).status===401,'Old temporary password rejected after change');
  const adminLogin=await request('/api/auth/login',{email:newAdmin.data.credentials.phone,password:adminPassword});
  ...
  const createBody={name:'测试业务员',phone:'13800000002',roleId:role.id};
  ...
  ok(member.status===201 && member.data.credentials.phone!==newAdmin.data.credentials.phone,'Administrator creates a member bound to a unique phone');
  const [dbRows]=await control.query(`SELECT password_hash,must_change_password FROM \`${db}\`.users WHERE phone=?`,[member.data.credentials.phone]);
  ...
  const memberLogin=await request('/api/auth/login',{email:member.data.credentials.phone,password:member.data.credentials.password});
  ...
  const sales=await request('/api/auth/login',{email:member.data.credentials.phone,password:memberPassword});
  ...
  await page.locator('#loginEmail').fill(newAdmin.data.credentials.phone);
  ...
  await page.locator('#iamNewMemberPhone').fill('13800000003');
  ...
  const uiPhone=info.match(/手机号：(.*)/)[1], uiPassword=info.match(/初始密码：(.*)/)[1];
  ...
  await signupPage.locator('#loginEmail').fill(uiPhone);await signupPage.locator('#loginPassword').fill(uiPassword);await signupPage.locator('#loginButton').click();
