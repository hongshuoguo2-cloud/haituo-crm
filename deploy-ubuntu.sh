...
if [[ -f "$ENV_FILE" ]]; then
  ...
  [[ -n "$EMAIL_TRACKING_SECRET" ]] || EMAIL_TRACKING_SECRET="$(read_existing_env_value EMAIL_TRACKING_SECRET)"
...
[[ -n "$EMAIL_TRACKING_SECRET" ]] || EMAIL_TRACKING_SECRET="$(generate_password)"
...
(( ${#EMAIL_TRACKING_SECRET} >= 32 )) || die "EMAIL_TRACKING_SECRET 至少需要 32 位"
...
apt-get update
apt-get install -y ca-certificates curl gnupg nginx mysql-server rsync python3 libreoffice-calc libreoffice-common fonts-noto-cjk
...
EMAIL_TRACKING_BASE_URL=$([[ -n "$DOMAIN" ]] && is_true "$ENABLE_HTTPS" && printf 'https://%s' "$DOMAIN")
EMAIL_MESSAGE_ID_DOMAIN=${DOMAIN:-goodjob-crm.local}
