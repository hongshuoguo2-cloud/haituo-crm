...
管理员凭据保存在 ../Haituo-local/登录信息.txt；验证开户生成的业务员凭据在 ../Haituo-local/演示用户.txt。登录邮箱是账号标识，不代表真实邮件账户。使用管理员账号进入“权限管理 → 成员管理”创建用户，不开放自主注册，也不在登录页公布密码。
...
本轮未宣称全部继承业务功能已验收。原接口盘点中的限制仍适用，包括未配置的 AI/搜客服务与旧物流模拟实现。管理员一键开户生成临时密码，新账号首次登录必须设置自己的密码；API 余额只部署入口，充值和扣费尚未接入。
