...
海拓基于 GoodJob CRM 二次开发，保留业务功能与管理员开户流程，完成独立品牌、登录会话、本机运行环境和 Windows 云服务器部署。演示地址为 https://demo.linqiagent.cn 。

## 本机使用

双击外层目录中的“启动海拓.cmd”。网址：http://127.0.0.1:5288/ 。

管理员凭据保存在 ../Haituo-local/登录信息.txt；验证开户生成的业务员凭据在 ../Haituo-local/演示用户.txt。新用户使用本人手机号作为登录账号；历史邮箱账号继续兼容。使用管理员账号进入“权限管理 → 成员管理”创建用户，不开放自主注册，也不在登录页公布密码。
...
本轮未宣称全部继承业务功能已验收。原接口盘点中的限制仍适用，包括未配置的 AI/搜客服务与旧物流模拟实现。管理员录入使用者手机号开户，系统生成一次性初始密码，新账号首次登录必须设置自己的密码；API 余额只部署入口，充值和扣费尚未接入。
